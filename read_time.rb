...
# Note: this requires img tag to be in the form of <img ... />. If using this
# on a post listing page, make sure to markdownify the post content first.
module ReadTimeFilter
    def read_time(input)
        ...
        (strings.size - 2).times do |i|
            image_time = (img_time_max - i) > img_time_min ? (img_time_max - i) : img_time_min
        end
